...
async function exportIncidentesToPDF(
  data,
  filename = "incidentes_gallina.pdf"
) {
  try {
    ...
    if (activeFechaInicio && activeFechaFin) {
      doc.text(
        `Rango de fechas: ${activeFechaInicio} a ${activeFechaFin}`,
        14,
        infoY
      );
      infoY += 5;
...
async function exportIncidentesToExcel(
  data,
  filename = "incidentes_gallina.xlsx"
) {
  try {
    if (!window.XLSX) {
      await new Promise((resolve, reject) => {
