...
async function exportToExcel() {
  try {
    ...
    XLSX.writeFile(
      wb,
      `salvamentos_${new Date().toISOString().split("T")[0]}.xlsx`
    );
...
async function exportToCSV() {
  try {
    ...
    const headers = [
      "ID Salvamento,Galpón,Fecha,Tipo Gallina,Cantidad Gallinas",
    ];
    ...
    link.setAttribute(
      "download",
      `salvamentos_${new Date().toISOString().split("T")[0]}.csv`
    );
